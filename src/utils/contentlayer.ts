...
export var dateSortDesc = (a: string, b: string) => {
  if (a > b) return -1;
  if (a < b) return 1;
  return 0;
};
...
export function sortedBlogPost(allBlogs: Blog[]) {
  var dateSortDesc = (a: string, b: string) => {
    if (a > b) return -1;
    if (a < b) return 1;
    return 0;
  };
 ...
 */
export function pick<Obj, Keys extends keyof Obj>(
  obj: Obj,
  keys: Keys[]
...
export function omit<Obj, Keys extends keyof Obj>(
  obj: Obj,
  keys: Keys[]
): Omit<Obj, Keys> {
